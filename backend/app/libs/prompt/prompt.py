...
PROMPT_GEN_HTML = """
帮我根据以下输入内容，生成一个 **HTML 片段** (fragment)。这个片段将被嵌入到一个已有的网页中，所以它本身不能是一个完整的 HTML 文档。

**核心要求**:

1.  **只生成 HTML 内容片段**: 输出的代码**绝对不能**包含 `<!DOCTYPE html>`, `<html>`, `<head>`, 或 `<body>` 标签。只需提供适合直接插入到父页面 `<body>` 元素内部的 HTML。
2.  **使用内联样式 (Inline Styles)**: **不要**包含 `<link>` 标签来引入外部 CSS (如 Tailwind, Font Awesome)。所有样式 (布局、颜色、字体大小、背景、渐变等) 必须使用 `style="..."` 属性直接应用在 HTML 元素上。
3.  **无外部脚本或复杂动画**: **不要**包含 `<script>` 标签，无论是内联脚本还是外部脚本链接 (`<script src="...">`)。**不要**尝试实现需要 JavaScript 的复杂动画 (如 Framer Motion 或 Apple 官网滚动效果)。生成的应是静态的、仅由 HTML 和内联 CSS 构成的片段。

**风格和内容要求**:

4.  **Bento Grid 风格**: 采用 Bento Grid 风格进行视觉布局 (使用内联 CSS 的 `display: grid` 等属性实现)。
5.  **视觉重点**: 使用超大字体或数字突出核心要点（通过内联 `font-size`, `font-weight` 等样式）。画面中应有超大视觉元素强调重点，与小元素形成视觉反差。中英文混用时，中文大字体粗体，英文小字点缀。可使用简洁的勾线图形化元素（最好是内联 SVG 或 Font Awesome **类名**）作为配图。
6.  **科技感渐变**: 运用单一高亮色的透明度渐变（通过内联 `background` 或 `color` 样式实现）来制造科技感。避免不同高亮色互相渐变。
7.  **图标**: 可以使用 Font Awesome 图标**类名** (例如 `<i class="fas fa-chart-line" style="..."></i>`)，但**不要**包含引入 Font Awesome CSS 的 `<link>` 标签。假设图标库已在宿主页面加载。避免使用 emoji 作为主要图标。
8.  **内容完整**: 确保输入内容中的所有要点都清晰地包含在生成的 HTML 片段中。

#输入内容
{text}

#输出格式 (仅包含 HTML 片段，无外层标签):
```html
HTML代码
```
"""
