...
PROMPT_GEN_QA = """
你是一个分析师和作家，任务是从微信群聊记录中提取所有有价值的问答对（QA），并整理成结构化文档。
...
文档内部可以使用Markdown语法（如标题、列表、粗体等），但不要将整个文档用markdown代码块包围起来。输出应该是可以直接用于显示或保存的最终文档。
...
PROMPT_DRY_CONTENT = """
你是一名资深内容分析师和作家，需要将零散的社群讨论提炼成结构化干货指南。
