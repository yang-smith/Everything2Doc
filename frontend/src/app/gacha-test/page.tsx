  ...
  const feedRobot = useCallback(async () => {
    ...
    try {
      ...
      if (currentClassification === CONTENT_TYPES.CHAT_LOG) {
        ...
        // --- Create the card frame HTML ---
        const html_content = `
          <div class="p-2 bg-amber-100 rounded-lg border-[6px] border-yellow-900 shadow-lg max-w-[400px] mx-auto">
            <img
              src="${img_url}"
              alt="Generated by AI"
              class="w-full max-h-[calc(70vh-1.5rem)] object-contain rounded-md block"
              loading="lazy"
            />
          </div>
        `;
            ...
            <>
              ...
              >
                ...
                <div className="bg-white rounded-lg p-4 sm:p-6 shadow-2xl border-4 border-[#D9C8B4] relative w-full max-w-3xl max-h-[90vh]"> {/* Increased max-width, max-height, reduced padding slightly */}
                    ...
                    <div ref={contentDisplayRef} className="mt-4 relative min-h-[200px] max-h-[75vh] overflow-y-auto p-1 rounded-md bg-white">
                        {error && (
                        ...
                        {generatedHtml && !parsedChatLogText && !error && (
                            <div ref={htmlDisplayRef} dangerouslySetInnerHTML={{ __html: generatedHtml }} />
                        )}
                    </div>
